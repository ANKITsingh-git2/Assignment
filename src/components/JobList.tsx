  ...
  const fetchJobs = async () => {
    try {
      let query = supabase
        ...
        .select(`
          *,
          profiles!jobs_manufacturer_id_fkey(name, location),
          manufacturer_details!inner(factory_name)
        `);
  ...
  const fetchApplications = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          profiles!job_applications_gig_worker_id_fkey(name, phone),
          gig_worker_details!inner(skills, work_type, experience_years)
        `)
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    {job.manufacturer_details[0]?.factory_name} • {job.profiles.location}
                  </CardDescription>
                </div>
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {app.gig_worker_details[0]?.work_type} • {app.gig_worker_details[0]?.experience_years} years experience
                                </p>
